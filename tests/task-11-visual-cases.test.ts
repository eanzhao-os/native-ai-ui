...
const TASK11_COMPONENT_SET = new Set<string>(TASK11_COMPONENTS);

const EXPECTED_CASES = {
  "audio-orb": ["initial", "settled", "recording", "muted", "ended", "focused"],
  "model-arena": ["initial", "settled", "voted-a", "voted-tie", "voted-b", "focused"],
  "insight-cards": [
    "initial",
    "settled",
    "compare-tooltip",
...
const INTERACTIVE_CASES = new Set([
  ...
  "audio-orb/focused",
  "model-arena/voted-a",
  "model-arena/voted-tie",
  "model-arena/voted-b",
  "model-arena/focused",
  "insight-cards/compare-tooltip",
  "insight-cards/anomaly",
  "insight-cards/usage",
  "insight-cards/usage-tooltip",
  "insight-cards/allocation-selected",
  "insight-cards/focused",
  "recommendation-card/alternatives-open",
  "recommendation-card/review-selected",
...
function analyzeTask11VisualSource(source: string) {
  ...
  const addRegistration = (
    ...
    if (
      ...
      !TASK11_COMPONENT_SET.has(component.text)
    ) {
      return;
    }
...
describe("Task 11 React visual cases", () => {
  test("defines every Multimodal & Arena state explicitly", () => {
    for (const component of TASK11_COMPONENTS) {
    ...
    const inventory = buildCaseInventory({
      componentIds: [...TASK11_COMPONENTS],
      themes: ["light", "dark"],
      locales: ["en", "zh"],
    });
    expect(inventory).toHaveLength(26 * 2 * 2);
  });

  test("backs every interactive Task 11 frame with a real action", () => {
    for (const component of TASK11_COMPONENTS) {
  ...
  test("detects reachable DOM fabrication and duplicate registrations", () => {
    const fixture = analyzeTask11VisualSource(`
      ...
      const CASES = new Map([
        ["insight-cards", [{ name: "fabricated", ...spreadAction }]],
      ]);
      CASES.set("insight-cards", [{ name: "override" }]);
    `);

    expect(fixture.duplicateComponents).toEqual(["insight-cards"]);
    expect(fixture.violations).toEqual(
      expect.arrayContaining([
        { component: "insight-cards", kind: "DOM evaluation" },
        { component: "insight-cards", kind: "DOM rewrite" },
      ]),
    );
  });
  ...
  test("accepts actions that drive live controls without rewriting capture DOM", () => {
    const fixture = analyzeTask11VisualSource(`
      async function action({ canvas, page }) {
        const control = canvas.getByRole("button", { name: "Accept" });
        await control.click();
        await control.focus();
        await page.keyboard.press("Enter");
        await canvas.getByText("Recommendation accepted").waitFor();
      }
      const CASES = new Map([
        ["recommendation-card", [{ name: "accepted", action }]],
      ]);
    `);
  ...
  test("keeps one honest reachable registration for every Task 11 component", () => {
    const source = readFileSync(resolve("tests/visual/cases.mjs"), "utf8");
    const analysis = analyzeTask11VisualSource(source);
